...
onAuthStateChanged(auth, async (user) => {
  ...
  const tomorrow = addDays(bdToday(), 1);
  const guestDateInput = document.getElementById("guestDate");
  guestDateInput.value = tomorrow;
  guestDateInput.min = tomorrow; // আজকে বা তার আগের তারিখে গেস্ট মিল দেয়া যাবে না
...
document.getElementById("addGuestBtn").addEventListener("click", async () => {
  ...
  if (!isEditableDate(dateStr)) {
    alert("আজকে বা তার আগের কোনো তারিখে গেস্ট মিল দেয়া যাবে না। আগের রাত ১১:৫৯:৫৯-এর মধ্যেই দিতে হয়।");
    return;
  }
...
pastToggleBtn.addEventListener("click", () => {
  ...
  pastToggleBtn.textContent = pastVisible ? "অতীতের মিল লুকাও" : "অতীতের মিল দেখো";
  pastList.style.display = pastVisible ? "block" : "none";
  if (pastVisible) renderPastView();
});
...
function renderPastView() {
  ...
  while (d < today) {
    const resolved = resolveSelection(savedEntriesMap, d, cycle.startDate);
    html += `
      <div class="day-card" style="display:flex; justify-content:space-between; align-items:center; padding:12px 14px;">
        <span>${formatBanglaDateFull(d)}</span>
        <span>
          ${resolved.isCarried ? '<span class="carried-tag">অটো</span>' : ""}
          <span class="view-tag">${MEAL_LABELS[resolved.value]}</span>
        </span>
      </div>`;
    d = addDays(d, 1);
  ...
  pastList.innerHTML = html || `<div class="empty-state">কোনো অতীত রেকর্ড নেই।</div>`;
}
